...
handler = logging.handlers.WatchedFileHandler(
    os.environ.get("LOGFILE", "logs/food_IL_ChampaignUrbana.log"))
...
if __name__ == '__main__':
	...
	for permit in range(1, 3000): 
		...
		info={}
		inspections={}
		url=('http://restaurants.cu-citizenaccess.org/restaurants/overview-data/?rest_permit=%s')%(permit)
		for trys in range(4):
			try:
		...
		r.set(('IL, ChampaignUrbana,%s')%(permit), pickle.dumps(page))
