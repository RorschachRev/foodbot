...
# TODO:
# Will need to Parse this out, aka: explain the variables, as well as
# provide a working example URL

URL:
http://www.inspectionsonline.us/MO/ClayLiberty/Inspect.nsf/SearchEstab?SearchView&Query=[fld_Program]+CONTAINS+kw_Food+AND+([fld_EstabName]+CONTAINS+%s+OR+[fld_FaciName]+CONTAINS+%s)&SearchOrder=4&SearchWV=TRUE&SearchFuzzy=FALSE

queryset: A-Z 0-9
Time Estimate: <ENTER TIME ESTIMATE HERE>
Date: <ENTER DATE HERE>
Name: <ENTER NAME HERE>
'''

handler = logging.handlers.WatchedFileHandler(
    os.environ.get("LOGFILE", "../logs/food_MO_Clay.log"))
...
if __name__ == '__main__':
	...
	for val in queryset:
		data=False
		pageurl=('http://www.inspectionsonline.us/MO/ClayLiberty/Inspect.nsf/SearchEstab?SearchView&Query=[fld_Program]+CONTAINS+kw_Food+AND+([fld_EstabName]+CONTAINS+%s+OR+[fld_FaciName]+CONTAINS+%s)&SearchOrder=4&SearchWV=TRUE&SearchFuzzy=FALSE')%(val,val)
	...
	for line in inspecturls:
		...
		try:
			...
			if(r.get( ('canMO_Clay_fac_%s')%(data['FacID']) )):
		...
		for val in range(0, inspection.__len__(), 3):	#3 td elements per tr, but no </tr>
			try:
				...
				inspec['InspURL']=('http://www.inspectionsonline.us/MO/ClayLiberty/Inspect.nsf/%s') % (x)
		...
		r.set(('canMO_Clay_fac_%s')%(data['FacID']), pickle.dumps(data))
