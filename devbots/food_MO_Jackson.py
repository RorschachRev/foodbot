...
handler = logging.handlers.WatchedFileHandler(
    os.environ.get("LOGFILE", "../logs/food_MO_Jackson.log"))
...
if __name__ == '__main__':
	...
	for line in inspecturls:
		...
		try:
			...
			if(r.get( ('canMO_Jackson_fac_%s')%(data['FacID']) )):
		...
		for val in range(0, inspection.__len__(), 3):	#3 td elements per tr, but no </tr>
			try:
				...
				inspec['InspURL']=('http://www.inspectionsonline.us/MO/USAKansasCity/Inspect.nsf/%s') % (x)
		...
		r.set(('canMO_Jackson_fac_%s')%(data['FacID']), pickle.dumps(data))
