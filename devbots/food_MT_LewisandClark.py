...
handler = logging.handlers.WatchedFileHandler(
    os.environ.get("LOGFILE", "../logs/food_MT_LewisandClark.log"))
...
if __name__ == '__main__':
	...
	for val in queryset:
		data=False
		pageurl=('http://www.inspectionsonline.us/MT/LewisandClarkHelena/Inspect.nsf/SearchEstab?SearchView&Query=[fld_Program]+CONTAINS+kw_Food+AND+([fld_EstabName]+CONTAINS+%s+OR+[fld_FaciName]+CONTAINS+%s)&SearchOrder=4&SearchWV=TRUE&SearchFuzzy=FALSE')%(val,val)
		...
		if data:
			...
			for value in data:
				...
				url=('https://www.inspectionsonline.us%s')%(href)
	...
	for line in inspecturls:
		...
		try:
			...
			if(r.get( ('canMT_LewisandClark_fac_%s')%(data['FacID']) )):
		...
		for val in range(0, inspection.__len__(), 3):	#3 td elements per tr, but no </tr>
			try:
				...
				inspec['InspURL']=('http://www.inspectionsonline.us/MT/LewisandClarkHelena/Inspect.nsf/%s') % (x)
		...
		r.set(('canMT_LewisandClark_fac_%s')%(data['FacID']), pickle.dumps(data))