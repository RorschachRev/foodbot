...
if __name__ == '__main__':
	...
	for line in inspecturls:
		...
		try:
			...
			if(r.get( ('canID_Bonneville_fac_%s')%(data['FacID']) )):
				print("Data exists:", data['FacID'])
				continue
			data['FacName']=details[1].getText().strip()
		...
		for val in range(0, inspection.__len__(), 3):	#3 td elements per tr, but no </tr>
			try:
				...
				inspec['InspURL']=('http://www.inspectionsonline.us/ID/bonnevilleidahofalls/Inspect.nsf/%s') % (x)
		...
		r.set(('canID_Bonneville_fac_%s')%(data['FacID']), pickle.dumps(data))
