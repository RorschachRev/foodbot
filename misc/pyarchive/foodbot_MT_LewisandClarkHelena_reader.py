...
handler = logging.handlers.WatchedFileHandler(
    os.environ.get("LOGFILE", "food_MT_LewisandClarkHelena_reader.log"))
...
if __name__ == '__main__':
	...
	f=open("MT_LewisandClarkHelena_queryurl.txt")
	donekeys=r.keys('MT_LewisandClarkHelena_reader_*')
	...
	#~ for line in ['/ID/BonnevilleIdahoFalls/Inspect.nsf/vw_InspectionsPubSumm?OpenView&RestrictToCategory=FC9D653476B24CA486257A5A0052C173']:
		...
		keyname=('MT_LewisandClarkHelena_reader_%s')%(bizid)
		...
		if data != None:
			...
			if inspections != {}:
				...
				r.set('MT_LewisandClarkHelena_reader_'+bizid, pickle.dumps(inspections[inspectid])) 
			#~ time.sleep(2)
		else:
