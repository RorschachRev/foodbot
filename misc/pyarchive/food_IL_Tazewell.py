...
handler = logging.handlers.WatchedFileHandler(
    os.environ.get("LOGFILE", "food_IL_Tazewell.log"))
...
if __name__ == '__main__':
	...
	for permitID in range(1,2000):
		if r.get(('IL_Tazewell_%s')%(permitID)):
			continue
		data={}
		inspections={}
		...
		try:
			...
			while val < info.__len__():
				...
				inspec['Url']=('https://il.healthinspections.us%s')%(details[0].find('a').get('href').split()[1])
				inspec['Demerits']=details[1].getText().split('\n')[7].strip()
				inspec['Type']=details[1].getText().split('\n')[10].split(':')[1].strip()
				vio=[]
				for var in range(val+2, details.__len__() - 1):
					vio.append(details[var].getText().strip())
				inspec['Violations']=vio
			...
			r.set(('IL_Tazewell_%s')%(permitID),pickle.dumps(data))
		except Exception:
