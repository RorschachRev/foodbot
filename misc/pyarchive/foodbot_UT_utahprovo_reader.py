...
handler = logging.handlers.WatchedFileHandler(
    os.environ.get("LOGFILE", "food_UT_utahprovo_reader.log"))
...
if __name__ == '__main__':
	...
	f=open("UT_utahprovo_queryurl.txt")
	donekeys=r.keys('UT_utahprovo_reader_*')
	...
	#~ for line in ['/ID/BonnevilleIdahoFalls/Inspect.nsf/vw_InspectionsPubSumm?OpenView&RestrictToCategory=FC9D653476B24CA486257A5A0052C173']:
		...
		keyname=('UT_utahprovo_reader_%s')%(bizid)
		...
		if data != None:
			...
			if inspections != {}:
				...
				r.set('UT_utahprovo_reader_'+bizid, pickle.dumps(inspections[inspectid])) 
			#~ time.sleep(2)
		else:
